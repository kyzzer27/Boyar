...
export default function May2026MonthlyCostPage() {
  ...
  useEffect(() => {
    ...
    const initChart = () => {
      ...
      if ((window as any)._monthlyChart_MAY_CATEGORIES) {
        ((window as any)._monthlyChart_MAY_CATEGORIES).destroy();
      }
      
      const el = document.getElementById('pie-chart') as HTMLCanvasElement;
      if (!el) return false;
      
      (window as any)._monthlyChart_MAY_CATEGORIES = new (window as any).Chart(el.getContext('2d'), {
        type: 'doughnut',
        data: {
          labels: MAY_CATEGORIES.map(c => c.name),
          datasets: [{
            data: MAY_CATEGORIES.map(c => c.inr),
            backgroundColor: MAY_CATEGORIES.map(c => c.color),
            borderColor: '#0F172A',
            borderWidth: 2,
            hoverOffset: 4
          }]
    ...
    return () => {
      ...
      if ((window as any)._monthlyChart_MAY_CATEGORIES) {
        ((window as any)._monthlyChart_MAY_CATEGORIES).destroy();
        (window as any)._monthlyChart_MAY_CATEGORIES = null;
      }
    }
  }, []);
  
  const pieSlices = MAY_CATEGORIES.map(cat => ({
