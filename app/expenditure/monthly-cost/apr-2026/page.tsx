...
const APR_CATEGORIES: AprCategory[] = [
  {
    ...
    color: "#3B82F6",
    description:
      "The software stack remains unchanged, indicating that tools selected earlier are sufficient and fit-for-purpose. April focuses on maintaining operational efficiency.",
    includes: [
      "ChatGPT Business ×2",
      "HubSpot Starter ×2",
      "Canva Pro",
  ...
  },
  {
    ...
    color: "#8B5CF6",
    description:
      "Rental continues to be more capital-efficient than ownership at this stage. No upgrades or additional devices required in April.",
  ...
  },
  {
    ...
    color: "#EF4444",
    description:
      "Content cadence and production quality remain consistent. April maintains steady output aligned with boutique positioning.",
  ...
  },
  {
    ...
    color: "#F59E0B",
    description:
      "April typically sees lower immediate decision-making activity among HNIs and corporates following fiscal year-end execution. Advertising spend is intentionally reduced to prioritise efficiency over volume. The focus during this month is on retargeting, nurturing high-intent leads generated earlier, and optimising CAC rather than aggressive top-of-funnel expansion.",
  },
  {
...
export default function Apr2026MonthlyCostPage() {
  const router = useRouter();
    const [hoveredIndex, setHoveredIndex] = useState<number | null>(null);
  const [lineHoverIndex, setLineHoverIndex] = useState<number | null>(null);

  const totalInr = APR_CATEGORIES.reduce((sum, c) => sum + c.inr, 0);
  const totalUsd = APR_CATEGORIES.reduce((sum, c) => sum + c.usd, 0);
  ...
  useEffect(() => {
    ...
    const initChart = () => {
      ...
      if ((window as any)._monthlyChart_APR_CATEGORIES) {
        ((window as any)._monthlyChart_APR_CATEGORIES).destroy();
      }
      
      const el = document.getElementById('pie-chart') as HTMLCanvasElement;
      if (!el) return false;
      
      (window as any)._monthlyChart_APR_CATEGORIES = new (window as any).Chart(el.getContext('2d'), {
        type: 'doughnut',
        data: {
          labels: APR_CATEGORIES.map(c => c.name),
          datasets: [{
            data: APR_CATEGORIES.map(c => c.inr),
            backgroundColor: APR_CATEGORIES.map(c => c.color),
            borderColor: '#0F172A',
            borderWidth: 2,
            hoverOffset: 4
          }]
    ...
    return () => {
      ...
      if ((window as any)._monthlyChart_APR_CATEGORIES) {
        ((window as any)._monthlyChart_APR_CATEGORIES).destroy();
        (window as any)._monthlyChart_APR_CATEGORIES = null;
      }
    }
  }, []);
  
  const pieSlices = APR_CATEGORIES.map(cat => ({
        ...
        <header className="relative z-10 border-b border-white/10 bg-black/90">
          <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-4 px-4 py-4 sm:px-6 lg:px-8">
            ...
            <div className="text-center flex-1">
              ...
              >
                April 2026 — Monthly Expenditure Breakdown
              </h1>
            </div>
            <div className="text-right text-xs text-white/60">
              <p>Exchange Rate: 1 USD = 91 INR</p>
              <p>Nature: Post–year-end optimisation month</p>
            </div>
          </div>
        </header>
        ...
        <main className="relative z-10 mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8 space-y-10">
            ...
            <div className="grid gap-6 md:grid-cols-[1.6fr,1.1fr]">
              <div className="space-y-4 text-sm sm:text-base leading-relaxed text-white/80">
                <p className="text-white/80">
                  April 2026 represents a post–year-end optimisation month. Following the
                  fiscal year-end execution period, decision-making activity among HNIs and
                  corporates typically decreases. Marketing spend is intentionally reduced
                  to prioritise efficiency, retargeting, and CAC optimisation over volume.
                </p>
                <p className="text-white/80">
                  The focus shifts to nurturing high-intent leads generated during the
                  March push, maintaining pipeline hygiene, and optimising conversion
                  efficiency rather than aggressive top-of-funnel expansion. All other
                  categories remain stable, indicating operational maturity and fit-for-purpose
                  infrastructure.
                </p>
              </div>
              <div className="grid gap-3 text-sm">
                ...
                <div className="rounded-2xl border border-white/10 bg-black/40 p-4">
                  ...
                  <p className="mt-2 text-sm text-white/80">
                    No new licensing or legal costs. Marketing spend reduced to prioritise
                    efficiency and retargeting. Travel & networking remain variable.
                  </p>
                </div>
              </div>
            </div>
          ...
          <section className="grid gap-8 lg:grid-cols-[1.2fr,1.1fr]">
            ...
            >
              ...
              <p className="text-xs text-white/60 mb-6">
                Animated month-level allocation of April burn across operating
                categories.
              </p>
                  ...
                  {hoveredIndex !== null && (
                    <div className="mt-3 rounded-2xl border border-white/15 bg-black/80 p-3 text-[11px] sm:text-xs text-white/80">
                      <p className="mb-1 font-semibold" style={{ fontFamily: "var(--font-benzin)" }}>
                        {APR_CATEGORIES[hoveredIndex].name}
                      </p>
                      <p>{APR_CATEGORIES[hoveredIndex].description}</p>
                    </div>
                  )}
                </div>
              </div>
          ...
          >
            <h2
              ...
              style={{ fontFamily: "var(--font-benzin)" }}
            >
              April Cost Curve by Category
            </h2>
            <p className="text-xs text-white/60 mb-6">
              Professional line visualization of how April spend builds up across
            ...
            <div className="w-full max-w-3xl mx-auto">
              <div className="relative h-64 w-full">
                ...
                {lineHoverIndex !== null && (
                  ...
                  >
                    <p
                      ...
                      style={{ fontFamily: "var(--font-benzin)" }}
                    >
                      {APR_CATEGORIES[lineHoverIndex].name}
                    </p>
                    <p className="mb-1">
                      {formatInr(APR_CATEGORIES[lineHoverIndex].inr)} ·{" "}
                      {formatUsd(APR_CATEGORIES[lineHoverIndex].usd)}
                    </p>
                    <p className="text-white/70">
                      {APR_CATEGORIES[lineHoverIndex].description}
                    </p>
                  </div>
                )}
                ...
                <svg viewBox="0 0 320 180" className="h-full w-full">
                  ...
                  {(() => {
                    const max = totalInr;
                    const step = (310 - 32) / (APR_CATEGORIES.length - 1 || 1);
                    const points = APR_CATEGORIES.map((cat, idx) => {
                      const x = 32 + step * idx;
                    ...
                    return (
                      <>
                        ...
                        {points.map((p, idx) => (
                          <motion.circle
                            key={APR_CATEGORIES[idx].name}
                            cx={p.x}
                            cy={p.y}
                            r={4}
                            fill={APR_CATEGORIES[idx].color}
                            stroke="#0F172A"
                            strokeWidth={1}
              ...
              <div className="mt-4 flex flex-wrap justify-between gap-4 text-[10px] sm:text-xs text-white/60">
                <p>
                  April shows post–year-end optimisation: marketing spend reduced to
                  prioritise efficiency, retargeting, and CAC optimisation, while all
          ...
          >
            <h2
              ...
              style={{ fontFamily: "var(--font-benzin)" }}
            >
              Category-by-Category View — April 2026
            </h2>
                  ...
                  {category.name === "Marketing & Ads" && (
                    <div className="rounded-xl border border-blue-400/30 bg-blue-400/10 p-3">
                      <p className="text-[10px] text-blue-200 mb-2">
                        <span className="font-semibold">Change vs March:</span>{" "}
                        ₹40,000 reduction
                      </p>
                      <p className="text-[10px] text-blue-200/90 leading-relaxed">
                        April typically sees lower immediate decision-making activity
                        among HNIs and corporates following fiscal year-end execution.
                        Advertising spend is intentionally reduced to prioritise efficiency
                        over volume.
                      </p>
                    </div>
                  )}
            ...
            <div className="mt-6 rounded-2xl border border-blue-400/30 bg-blue-400/10 p-4">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-white/70">
                    Variable | Not fixed. April may include selective client meetings
          ...
          <section className="grid gap-8 lg:grid-cols-[1.2fr,1.1fr]">
            ...
            >
              <h2
                ...
                style={{ fontFamily: "var(--font-benzin)" }}
              >
                April 2026 — Category-Wise Spend Table
              </h2>
                ...
                <table className="w-full text-xs sm:text-sm">
                  ...
                  <tbody>
                    ...
                    <tr className="border-t border-white/10 text-white font-semibold">
                      <td className="px-4 py-2">Total (April 2026)</td>
                      <td className="px-4 py-2 text-right">
                        {formatInr(totalInr)}
                      </td>
                      <td className="px-4 py-2 text-right">
            ...
            >
              <p className="text-xs uppercase tracking-[0.4em] text-white/60 mb-2">
                Investor Summary — April 2026
              </p>
              <p className="text-sm text-white/80 leading-relaxed">
                April reflects post–year-end optimisation with marketing spend reduced
                to prioritise efficiency, retargeting, and CAC optimisation. The focus
                shifts to nurturing high-intent leads generated during March rather than
                aggressive top-of-funnel expansion.
              </p>
              <p className="mt-3 text-sm text-white/80 leading-relaxed">
                All other categories remain stable, indicating operational maturity and
                fit-for-purpose infrastructure. Travel & networking expenses remain
