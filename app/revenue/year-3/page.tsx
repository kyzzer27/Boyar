...
const revenueRows: RevenueRow[] = [
  { service: "Company Formation", clients: 10, unitUsd: 2500, newRevenue: 25000, renewals: 15, renewalRevenue: 26250, total: 51250 },
  { service: "Standalone Trust", clients: 2, unitUsd: 13000, newRevenue: 26000, renewals: 2, renewalRevenue: 18200, total: 44200 },
  { service: "Full Structure", clients: 3, unitUsd: 29700, newRevenue: 89100, renewals: 5, renewalRevenue: 103950, total: 193050 },
...
const TOTAL_REVENUE_USD = 315500;
const TOTAL_REVENUE_INR = Math.round(315500 * 90.44);
const TOTAL_CLIENTS = revenueRows.reduce((sum, row) => sum + row.clients, 0);
const GROWTH_VS_YEAR2 = "48.3%";
  ...
  return (
    ...
    >
      ...
      <h3 className="mt-2 text-lg font-semibold text-white" style={{ fontFamily: "var(--font-benzin)" }}>
        Year 3 Performance by Service (USD)
      </h3>
...
export default function YearThreeRevenuePage() {
      ...
      <div className="relative min-h-screen bg-black text-white">
        ...
        <header className="relative z-10 border-b border-white/10 bg-black/90">
          <div className="mx-auto flex max-w-6xl flex-wrap items-center justify-between gap-4 px-4 py-5 sm:px-6 lg:px-8">
            ...
            <div>
              <p className="text-xs uppercase tracking-[0.35em] text-white/50">Year 3 Projection</p>
              <h1 className="text-2xl font-semibold text-white" style={{ fontFamily: "var(--font-benzin)" }}>
                Revenue Stack · FY2028
              </h1>
            </div>
            <div className="text-right text-xs text-white/60">
              <p>Total Clients: {TOTAL_CLIENTS}</p>
              <p>Growth vs Year 2: {GROWTH_VS_YEAR2}</p>
            </div>
          </div>
        </header>
          ...
          <section className="mt-10 rounded-3xl border border-white/10 bg-black/60 p-6 shadow-2xl">
            <div className="flex flex-wrap items-center justify-between gap-4">
              ...
              <div className="rounded-full border border-white/20 bg-white/5 px-4 py-2 text-xs text-white/70">
                Renewals dominate ARPU lift from Year 2 → Year 3.
              </div>
            </div>
            <div className="mt-6 overflow-x-auto">
              <table className="w-full text-sm text-white/80">
                ...
                <tbody>
                  ...
                  <tr className="border-t border-white/10 text-white font-semibold">
                    <td className="px-4 py-3">GROWTH % vs Year 2</td>
                    <td className="px-4 py-3 text-right">—</td>
                    <td className="px-4 py-3 text-right">—</td>
                    <td className="px-4 py-3 text-right">—</td>
                    <td className="px-4 py-3 text-right">—</td>
                    <td className="px-4 py-3 text-right">—</td>
                    <td className="px-4 py-3 text-right">{GROWTH_VS_YEAR2}</td>
                  </tr>
                </tbody>
              </table>
            </div>
          ...
          <section className="mt-10 rounded-3xl border border-white/10 bg-white/5 p-6">
            ...
            <div className="mt-4 grid gap-6 sm:grid-cols-2">
              {[
                {
                  title: "Renewal lift",
                  body: "26 renewal mandates generate $130K, showing low churn and operational stickiness.",
                },
                {
                  title: "ARPU expansion",
                  body: `Revenue grows ${GROWTH_VS_YEAR2} versus Year 2 while client count stays flat, highlighting upsell efficiency.`,
                },
                {
                  title: "FX reserves",
                  body: `USD ${usdFormatter(TOTAL_REVENUE_USD)} translates to ${inrFormatter(TOTAL_REVENUE_INR)}, funding Boyar's global footprint.`,
                },
                {
                  title: "Forward plan",
                  body: "Year 3 sets the stage for combined projections that layer DAO solutions, FATCA / CRS retainers, and productized compliance subscriptions.",
                },
              ].map((block) => (
