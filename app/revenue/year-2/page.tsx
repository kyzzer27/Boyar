...
const revenueRows: RevenueRow[] = [
  { service: "Company Formation", clients: 9, unitUsd: 2500, newRevenue: 22500, renewals: 6, renewalRevenue: 10500, total: 33000 },
  { service: "Standalone Trust", clients: 1, unitUsd: 13000, newRevenue: 13000, renewals: 1, renewalRevenue: 9100, total: 22100 },
  { service: "Full Structure", clients: 3, unitUsd: 29700, newRevenue: 89100, renewals: 2, renewalRevenue: 41580, total: 130680 },
...
const TOTAL_REVENUE_USD = 212780;
const TOTAL_REVENUE_INR = Math.round(212780 * 90.44);
const TOTAL_CLIENTS = revenueRows.reduce((sum, row) => sum + row.clients, 0);
const GROWTH_VS_YEAR1 = "91.9%";
  ...
  return (
    ...
    >
      ...
      <h3 className="mt-2 text-lg font-semibold text-white" style={{ fontFamily: "var(--font-benzin)" }}>
        Year 2 Performance by Service (USD)
      </h3>
          ...
          <div className="mx-auto flex max-w-6xl flex-wrap items-center justify-between gap-4 px-4 py-5 sm:px-6 lg:px-8">
            ...
            <div>
              ...
              <h1 className="text-2xl font-semibold text-white" style={{ fontFamily: "var(--font-benzin)" }}>
                Revenue Stack · FY2027
              </h1>
            </div>
            <div className="text-right text-xs text-white/60">
              <p>Total Clients: {TOTAL_CLIENTS}</p>
              <p>Growth vs Year 1: {GROWTH_VS_YEAR1}</p>
            </div>
          </div>
        </header>
          ...
          <section className="mt-10 rounded-3xl border border-white/10 bg-black/60 p-6 shadow-2xl">
            <div className="flex flex-wrap items-center justify-between gap-4">
              ...
              <div className="rounded-full border border-white/20 bg-white/5 px-4 py-2 text-xs text-white/70">
                Renewals kick in from Company Formation & Full Structure.
              </div>
            </div>
            <div className="mt-6 overflow-x-auto">
              <table className="w-full text-sm text-white/80">
                ...
                <tbody>
                  ...
                  <tr className="border-t border-white/10 text-white font-semibold">
                    <td className="px-4 py-3">GROWTH % vs Year 1</td>
                    <td className="px-4 py-3 text-right">—</td>
                    <td className="px-4 py-3 text-right">—</td>
                    <td className="px-4 py-3 text-right">—</td>
                    <td className="px-4 py-3 text-right">—</td>
                    <td className="px-4 py-3 text-right">—</td>
                    <td className="px-4 py-3 text-right">{GROWTH_VS_YEAR1}</td>
                  </tr>
                </tbody>
              </table>
            </div>
              ...
              {[
                {
                  ...
                  body: "Company Formation and Full Structure renewals add $59.6K to topline, representing 31% of total revenue.",
                },
                {
                  title: "Growth trajectory",
                  body: `Revenue scales ${GROWTH_VS_YEAR1} versus Year 1, while client count increases 41%, showing higher ARPU.`,
                },
                {
                  title: "FX leverage",
                  body: `USD ${usdFormatter(TOTAL_REVENUE_USD)} converts to ${inrFormatter(TOTAL_REVENUE_INR)}, financing India HQ expansion.`,
                },
                {
                  title: "Future pipeline",
                  body: "Year 2 sets the baseline for multiyear recurring revenue. Year 3 onwards will unlock retainer overlays and ancillary upsells.",
                },
              ].map((block) => (
