...
const revenueRows: RevenueRow[] = [
  { service: "Company Formation", clients: 18, unitUsd: 2500, newRevenue: 45000, renewals: 34, renewalRevenue: 63000, total: 108000 },
  { service: "Standalone Trust", clients: 3, unitUsd: 13000, newRevenue: 39000, renewals: 5, renewalRevenue: 45500, total: 84500 },
  { service: "Full Structure", clients: 6, unitUsd: 29700, newRevenue: 178200, renewals: 11, renewalRevenue: 236040, total: 414240 },
...
const TOTAL_REVENUE_USD = 672740;
const TOTAL_REVENUE_INR = Math.round(672740 * 90.44);
const TOTAL_CLIENTS = revenueRows.reduce((sum, row) => sum + row.clients, 0);
const GROWTH_VS_YEAR4 = "24.6%";
  ...
  return (
    ...
    >
      ...
      <h3 className="mt-2 text-lg font-semibold text-white" style={{ fontFamily: "var(--font-benzin)" }}>
        Year 5 Performance by Service (USD)
      </h3>
...
export default function YearFiveRevenuePage() {
      ...
      <div className="relative min-h-screen bg-black text-white">
        ...
        <header className="relative z-10 border-b border-white/10 bg-black/90">
          <div className="mx-auto flex max-w-6xl flex-wrap items-center justify-between gap-4 px-4 py-5 sm:px-6 lg:px-8">
            ...
            <div>
              <p className="text-xs uppercase tracking-[0.35em] text-white/50">Year 5 Projection</p>
              <h1 className="text-2xl font-semibold text-white" style={{ fontFamily: "var(--font-benzin)" }}>
                Revenue Stack · FY2030
              </h1>
            </div>
            <div className="text-right text-xs text-white/60">
              <p>Total Clients: {TOTAL_CLIENTS}</p>
              <p>Growth vs Year 4: {GROWTH_VS_YEAR4}</p>
            </div>
          </div>
        </header>
          ...
          <section className="mt-10 rounded-3xl border border-white/10 bg-black/60 p-6 shadow-2xl">
            <div className="flex flex-wrap items-center justify-between gap-4">
              ...
              <div className="rounded-full border border-white/20 bg-white/5 px-4 py-2 text-xs text-white/70">
                Renewal layer funds productized compliance roadmap.
              </div>
            </div>
            <div className="mt-6 overflow-x-auto">
              <table className="w-full text-sm text-white/80">
                ...
                <tbody>
                  ...
                  <tr className="border-t border-white/10 text-white font-semibold">
                    <td className="px-4 py-3">GROWTH % vs Year 4</td>
                    <td className="px-4 py-3 text-right">—</td>
                    <td className="px-4 py-3 text-right">—</td>
                    <td className="px-4 py-3 text-right">—</td>
                    <td className="px-4 py-3 text-right">—</td>
                    <td className="px-4 py-3 text-right">—</td>
                    <td className="px-4 py-3 text-right">{GROWTH_VS_YEAR4}</td>
                  </tr>
                </tbody>
              </table>
            </div>
          ...
          <section className="mt-10 grid gap-6 lg:grid-cols-[1.1fr,0.9fr]">
            ...
            >
              ...
              <div className="mt-6 space-y-3 text-sm text-white/70">
                {revenueRows.map((row) => (
                  <div key={row.service} className="flex itemsцентер justify-between">
          ...
          <section className="mt-10 rounded-3xl border border-white/10 bg-white/5 p-6">
            ...
            <div className="mt-4 grid gap-6 sm:grid-cols-2">
              {[
                {
                  title: "Renewal moat",
                  body: "59 renewal mandates deliver $294K, locking in multi-year revenue visibility and reducing CAC burden.",
                },
                {
                  title: "YoY growth",
                  body: `Topline grows ${GROWTH_VS_YEAR4} vs Year 4 even with flat client intake, driven by retained structures.`,
                },
                {
                  title: "FX runway",
                  body: `USD ${usdFormatter(TOTAL_REVENUE_USD)} equals ${inrFormatter(TOTAL_REVENUE_INR)}, financing global expansion and DAO build-outs.`,
                },
                {
                  title: "Combined projection fit",
                  body: "Year 5 data feeds the combined view where DAO, AML, and FATCA / CRS retainers overlay on the renewal stack.",
                },
              ].map((block) => (
