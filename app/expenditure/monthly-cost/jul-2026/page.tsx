...
const JUL_CATEGORIES: JulyCategory[] = [
  {
    ...
    color: "#3B82F6",
    description:
      "The software stack remains unchanged, indicating that tools selected earlier are sufficient and fit-for-purpose. July focuses on maintaining operational efficiency.",
    includes: [
      "ChatGPT Business ×2",
      "HubSpot Starter ×2",
      "Canva Pro",
  ...
  },
  {
    ...
    color: "#8B5CF6",
    description:
      "Rental continues to be more capital-efficient than ownership at this stage. No upgrades or additional devices required in July.",
  ...
  },
  {
    ...
    color: "#EF4444",
    description:
      "Content cadence and production quality remain consistent. July maintains steady output aligned with boutique positioning.",
  ...
  },
  {
    ...
    color: "#F59E0B",
    description:
      "July is traditionally a softer month for advisory decision-making due to holidays, reduced executive availability, and slower corporate action. Ad spend is deliberately lowered to focus on maintaining baseline visibility, retargeting existing prospects, and controlling CAC. Capital is conserved for higher-intent months in H2 where conversion probability increases.",
  },
  {
...
export default function July2026MonthlyCostPage() {
  const router = useRouter();
    const [hoveredIndex, setHoveredIndex] = useState<number | null>(null);
  const [lineHoverIndex, setLineHoverIndex] = useState<number | null>(null);

  const totalInr = JUL_CATEGORIES.reduce((sum, c) => sum + c.inr, 0);
  const totalUsd = JUL_CATEGORIES.reduce((sum, c) => sum + c.usd, 0);
  ...
  useEffect(() => {
    ...
    const initChart = () => {
      ...
      if ((window as any)._monthlyChart_JUL_CATEGORIES) {
        ((window as any)._monthlyChart_JUL_CATEGORIES).destroy();
      }
      
      const el = document.getElementById('pie-chart') as HTMLCanvasElement;
      if (!el) return false;
      
      (window as any)._monthlyChart_JUL_CATEGORIES = new (window as any).Chart(el.getContext('2d'), {
        type: 'doughnut',
        data: {
          labels: JUL_CATEGORIES.map(c => c.name),
          datasets: [{
            data: JUL_CATEGORIES.map(c => c.inr),
            backgroundColor: JUL_CATEGORIES.map(c => c.color),
            borderColor: '#0F172A',
            borderWidth: 2,
            hoverOffset: 4
          }]
    ...
    return () => {
      ...
      if ((window as any)._monthlyChart_JUL_CATEGORIES) {
        ((window as any)._monthlyChart_JUL_CATEGORIES).destroy();
        (window as any)._monthlyChart_JUL_CATEGORIES = null;
      }
    }
  }, []);
  
  const pieSlices = JUL_CATEGORIES.map(cat => ({
        ...
        <header className="relative z-10 border-b border-white/10 bg-black/90">
          <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-4 px-4 py-4 sm:px-6 lg:px-8">
            ...
            <div className="text-center flex-1">
              ...
              >
                July 2026 — Monthly Expenditure Breakdown
              </h1>
            </div>
        ...
        <main className="relative z-10 mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8 space-y-10">
            ...
            <div className="grid gap-6 md:grid-cols-[1.6fr,1.1fr]">
              <div className="space-y-4 text-sm sm:text-base leading-relaxed text-white/80">
                <p className="text-white/80">
                  July 2026 represents a transition month where decision-makers begin
          ...
          <section className="grid gap-8 lg:grid-cols-[1.2fr,1.1fr]">
            ...
            >
              ...
              <p className="text-xs text-white/60 mb-6">
                Animated month-level allocation of July burn across operating
                categories.
              </p>
                  ...
                  {hoveredIndex !== null && (
                    <div className="mt-3 rounded-2xl border border-white/15 bg-black/80 p-3 text-[11px] sm:text-xs text-white/80">
                      <p className="mb-1 font-semibold" style={{ fontFamily: "var(--font-benzin)" }}>
                        {JUL_CATEGORIES[hoveredIndex].name}
                      </p>
                      <p>{JUL_CATEGORIES[hoveredIndex].description}</p>
                    </div>
                  )}
                </div>
              </div>
          ...
          >
            <h2
              ...
              style={{ fontFamily: "var(--font-benzin)" }}
            >
              July Cost Curve by Category
            </h2>
            <p className="text-xs text-white/60 mb-6">
              Professional line visualization of how July spend builds up across
            ...
            <div className="w-full max-w-3xl mx-auto">
              <div className="relative h-64 w-full">
                ...
                {lineHoverIndex !== null && (
                  ...
                  >
                    <p
                      ...
                      style={{ fontFamily: "var(--font-benzin)" }}
                    >
                      {JUL_CATEGORIES[lineHoverIndex].name}
                    </p>
                    <p className="mb-1">
                      {formatInr(JUL_CATEGORIES[lineHoverIndex].inr)} ·{" "}
                      {formatUsd(JUL_CATEGORIES[lineHoverIndex].usd)}
                    </p>
                    <p className="text-white/70">
                      {JUL_CATEGORIES[lineHoverIndex].description}
                    </p>
                  </div>
                )}
                ...
                <svg viewBox="0 0 320 180" className="h-full w-full">
                  ...
                  {(() => {
                    const max = totalInr;
                    const step = (310 - 32) / (JUL_CATEGORIES.length - 1 || 1);
                    const points = JUL_CATEGORIES.map((cat, idx) => {
                      const x = 32 + step * idx;
                    ...
                    return (
                      <>
                        ...
                        {points.map((p, idx) => (
                          <motion.circle
                            key={JUL_CATEGORIES[idx].name}
                            cx={p.x}
                            cy={p.y}
                            r={4}
                            fill={JUL_CATEGORIES[idx].color}
                            stroke="#0F172A"
                            strokeWidth={1}
              ...
              <div className="mt-4 flex flex-wrap justify-between gap-4 text-[10px] sm:text-xs text-white/60">
                <p>
                  July shows low-intensity demand: marketing spend lowered to maintain
          ...
          >
            <h2
              ...
              style={{ fontFamily: "var(--font-benzin)" }}
            >
              Category-by-Category View — July 2026
            </h2>
                  ...
                  {category.name === "Marketing & Ads" && (
                    <div className="rounded-xl border border-blue-400/30 bg-blue-400/10 p-3">
                      <p className="text-[10px] text-blue-200 mb-2">
                        <span className="font-semibold">Change vs June:</span>{" "}
                        ₹5,000 reduction
                      </p>
                      <p className="text-[10px] text-blue-200/90 leading-relaxed">
                        July is traditionally a softer month for advisory decision-making due
                        to holidays, reduced executive availability, and slower corporate action.
                        Ad spend is deliberately lowered to focus on maintaining baseline
                        visibility, retargeting existing prospects, and controlling CAC.
                      </p>
                    </div>
                  )}
            ...
            <div className="mt-6 rounded-2xl border border-blue-400/30 bg-blue-400/10 p-4">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-white/70">
                    Variable | Not fixed. July may include selective client meetings
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-white/60 mb-2 italic">
                    Boutique advisory firms with outsourced execution commonly see these in mid-year months
                  </p>
                  <p className="text-xs text-white/70 mb-2">
                    These expenses are deal-triggered, not structural, and would be disclosed
                    transparently if incurred:
                  </p>
                  <ul className="space-y-1 text-[11px] text-white/70 ml-4 list-disc">
                    <li>Client onboarding costs from external banks, EMIs, or registrars</li>
                    <li>Specialist advisory fees for jurisdiction-specific matters</li>
                    <li>Documentation or compliance review charges</li>
                    <li>High-probability travel for closing-stage discussions</li>
                  </ul>
                </div>
              </div>
            </div>
          ...
          <section className="grid gap-8 lg:grid-cols-[1.2fr,1.1fr]">
            ...
            >
              <h2
                ...
                style={{ fontFamily: "var(--font-benzin)" }}
              >
                July 2026 — Category-Wise Spend Table
              </h2>
                ...
                <table className="w-full text-xs sm:text-sm">
                  ...
                  <tbody>
                    ...
                    <tr className="border-t border-white/10 text-white font-semibold">
                      <td className="px-4 py-2">Total (July 2026)</td>
                      <td className="px-4 py-2 text-right">
                        {formatInr(totalInr)}
                      </td>
                      <td className="px-4 py-2 text-right">
            ...
            >
              <p className="text-xs uppercase tracking-[0.4em] text-white/60 mb-2">
                Investor Summary — July 2026
              </p>
              <p className="text-sm text-white/80 leading-relaxed">
                July reflects a transition month where decision-makers begin re-engaging
