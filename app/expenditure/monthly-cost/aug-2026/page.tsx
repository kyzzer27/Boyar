...
const AUG_CATEGORIES: AugustCategory[] = [
  {
    ...
    color: "#3B82F6",
    description:
      "The software stack remains unchanged, indicating that tools selected earlier are sufficient and fit-for-purpose. August focuses on maintaining operational efficiency.",
    includes: [
      "ChatGPT Business ×2",
      "HubSpot Starter ×2",
      "Canva Pro",
  ...
  },
  {
    ...
    color: "#8B5CF6",
    description:
      "Rental continues to be more capital-efficient than ownership at this stage. No upgrades or additional devices required in August.",
  ...
  },
  {
    ...
    color: "#EF4444",
    description:
      "Content cadence and production quality remain consistent. August maintains steady output aligned with boutique positioning.",
  ...
  },
  {
    ...
    color: "#F59E0B",
    description:
      "August marks the restart of planning activity as corporates and HNIs resume focus after mid-year slowdowns. Ad spend is increased to re-engage decision-makers, capture renewed intent, and rebuild pipeline momentum ahead of the peak year-end planning window. This spend is aimed at top-of-funnel reactivation and early-stage structuring conversations.",
  },
  {
...
export default function August2026MonthlyCostPage() {
  const router = useRouter();
    const [hoveredIndex, setHoveredIndex] = useState<number | null>(null);
  const [lineHoverIndex, setLineHoverIndex] = useState<number | null>(null);

  const totalInr = AUG_CATEGORIES.reduce((sum, c) => sum + c.inr, 0);
  const totalUsd = AUG_CATEGORIES.reduce((sum, c) => sum + c.usd, 0);
  ...
  useEffect(() => {
    ...
    const initChart = () => {
      ...
      if ((window as any)._monthlyChart_AUG_CATEGORIES) {
        ((window as any)._monthlyChart_AUG_CATEGORIES).destroy();
      }
      
      const el = document.getElementById('pie-chart') as HTMLCanvasElement;
      if (!el) return false;
      
      (window as any)._monthlyChart_AUG_CATEGORIES = new (window as any).Chart(el.getContext('2d'), {
        type: 'doughnut',
        data: {
          labels: AUG_CATEGORIES.map(c => c.name),
          datasets: [{
            data: AUG_CATEGORIES.map(c => c.inr),
            backgroundColor: AUG_CATEGORIES.map(c => c.color),
            borderColor: '#0F172A',
            borderWidth: 2,
            hoverOffset: 4
          }]
    ...
    return () => {
      ...
      if ((window as any)._monthlyChart_AUG_CATEGORIES) {
        ((window as any)._monthlyChart_AUG_CATEGORIES).destroy();
        (window as any)._monthlyChart_AUG_CATEGORIES = null;
      }
    }
  }, []);
  
  const pieSlices = AUG_CATEGORIES.map(cat => ({
        ...
        <header className="relative z-10 border-b border-white/10 bg-black/90">
          <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-4 px-4 py-4 sm:px-6 lg:px-8">
            ...
            <div className="text-center flex-1">
              ...
              >
                August 2026 — Monthly Expenditure Breakdown
              </h1>
            </div>
            <div className="text-right text-xs text-white/60">
              <p>Exchange Rate: 1 USD = 91 INR</p>
              <p>Nature: H2 restart, renewed planning activity</p>
            </div>
          </div>
        </header>
        ...
        <main className="relative z-10 mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8 space-y-10">
            ...
            <div className="grid gap-6 md:grid-cols-[1.6fr,1.1fr]">
              <div className="space-y-4 text-sm sm:text-base leading-relaxed text-white/80">
                <p className="text-white/80">
                  August 2026 marks the H2 restart with renewed planning activity. Corporates
                  and HNIs resume focus after mid-year slowdowns. Marketing spend is increased
                  to re-engage decision-makers, capture renewed intent, and rebuild pipeline
                  momentum ahead of the peak year-end planning window.
                </p>
                <p className="text-white/80">
                  This spend is aimed at top-of-funnel reactivation and early-stage structuring
                  conversations. All other categories remain stable, indicating operational
                  maturity and fit-for-purpose infrastructure.
                </p>
              </div>
              <div className="grid gap-3 text-sm">
                <div className="rounded-2xl border border-white/10 bg-black/40 p-4">
                  <p className="text-xs uppercase tracking-[0.3em] text-white/60">
                    Total Month Cost (August 2026)
                  </p>
                  <p
                ...
                <div className="rounded-2xl border border-white/10 bg-black/40 p-4">
                  ...
                  <p className="mt-2 text-sm text-white/80">
                    No new licensing or legal costs. Marketing spend increased to re-engage
                    decision-makers and rebuild pipeline momentum. Travel & networking remain variable.
                  </p>
                </div>
              </div>
            </div>
          ...
          <section className="grid gap-8 lg:grid-cols-[1.2fr,1.1fr]">
            ...
            >
              ...
              <p className="text-xs text-white/60 mb-6">
                Animated month-level allocation of August burn across operating
                categories.
              </p>
                  ...
                  {hoveredIndex !== null && (
                    <div className="mt-3 rounded-2xl border border-white/15 bg-black/80 p-3 text-[11px] sm:text-xs text-white/80">
                      <p className="mb-1 font-semibold" style={{ fontFamily: "var(--font-benzin)" }}>
                        {AUG_CATEGORIES[hoveredIndex].name}
                      </p>
                      <p>{AUG_CATEGORIES[hoveredIndex].description}</p>
                    </div>
                  )}
                </div>
              </div>
          ...
          >
            <h2
              ...
              style={{ fontFamily: "var(--font-benzin)" }}
            >
              August Cost Curve by Category
            </h2>
            <p className="text-xs text-white/60 mb-6">
              Professional line visualization of how August spend builds up across
            ...
            <div className="w-full max-w-3xl mx-auto">
              <div className="relative h-64 w-full">
                ...
                {lineHoverIndex !== null && (
                  ...
                  >
                    <p
                      ...
                      style={{ fontFamily: "var(--font-benzin)" }}
                    >
                      {AUG_CATEGORIES[lineHoverIndex].name}
                    </p>
                    <p className="mb-1">
                      {formatInr(AUG_CATEGORIES[lineHoverIndex].inr)} ·{" "}
                      {formatUsd(AUG_CATEGORIES[lineHoverIndex].usd)}
                    </p>
                    <p className="text-white/70">
                      {AUG_CATEGORIES[lineHoverIndex].description}
                    </p>
                  </div>
                )}
                ...
                <svg viewBox="0 0 320 180" className="h-full w-full">
                  ...
                  {(() => {
                    const max = totalInr;
                    const step = (310 - 32) / (AUG_CATEGORIES.length - 1 || 1);
                    const points = AUG_CATEGORIES.map((cat, idx) => {
                      const x = 32 + step * idx;
                    ...
                    return (
                      <>
                        ...
                        {points.map((p, idx) => (
                          <motion.circle
                            key={AUG_CATEGORIES[idx].name}
                            cx={p.x}
                            cy={p.y}
                            r={4}
                            fill={AUG_CATEGORIES[idx].color}
                            stroke="#0F172A"
                            strokeWidth={1}
              ...
              <div className="mt-4 flex flex-wrap justify-between gap-4 text-[10px] sm:text-xs text-white/60">
                <p>
                  August shows H2 restart: marketing spend increased to re-engage
                  decision-makers and rebuild pipeline momentum, while all other categories
                  remain stable.
                </p>
                <p>
          ...
          >
            <h2
              ...
              style={{ fontFamily: "var(--font-benzin)" }}
            >
              Category-by-Category View — August 2026
            </h2>
                  ...
                  {category.name === "Marketing & Ads" && (
                    <div className="rounded-xl border border-emerald-400/30 bg-emerald-400/10 p-3">
                      <p className="text-[10px] text-emerald-200 mb-2">
                        <span className="font-semibold">Change vs July:</span>{" "}
                        ₹30,000 increase
                      </p>
                      <p className="text-[10px] text-emerald-200/90 leading-relaxed">
                        August marks the restart of planning activity as corporates and HNIs
                        resume focus after mid-year slowdowns. Ad spend is increased to
                        re-engage decision-makers, capture renewed intent, and rebuild pipeline
                        momentum ahead of the peak year-end planning window.
                      </p>
                    </div>
                  )}
            ...
            <div className="mt-6 rounded-2xl border border-blue-400/30 bg-blue-400/10 p-4">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-white/70">
                    Variable | Not fixed. August may include selective client meetings
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-white/60 mb-2 italic">
                    Often observed in boutique advisory firms during H2 ramp-up
                  </p>
                  <p className="text-xs text-white/70 mb-2">
                    Any such expenses would be deal-driven, approved internally, and disclosed
                    transparently to investors:
                  </p>
                  <ul className="space-y-1 text-[11px] text-white/70 ml-4 list-disc">
                    <li>Initial onboarding costs from banking or trust service providers</li>
                    <li>Specialist advisory fees for complex structuring requests</li>
                    <li>Short-term travel for revived client meetings</li>
                    <li>Additional documentation or legal review charges</li>
                  </ul>
                </div>
              </div>
            </div>
          ...
          <section className="grid gap-8 lg:grid-cols-[1.2fr,1.1fr]">
            ...
            >
              <h2
                ...
                style={{ fontFamily: "var(--font-benzin)" }}
              >
                August 2026 — Category-Wise Spend Table
              </h2>
                ...
                <table className="w-full text-xs sm:text-sm">
                  ...
                  <tbody>
                    ...
                    <tr className="border-t border-white/10 text-white font-semibold">
                      <td className="px-4 py-2">Total (August 2026)</td>
                      <td className="px-4 py-2 text-right">
                        {formatInr(totalInr)}
                      </td>
                      <td className="px-4 py-2 text-right">
            ...
            >
              <p className="text-xs uppercase tracking-[0.4em] text-white/60 mb-2">
                Investor Summary — August 2026
              </p>
              <p className="text-sm text-white/80 leading-relaxed">
                August reflects a transition month where decision-makers begin re-engaging
