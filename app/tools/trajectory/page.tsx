...
const trajectoryButtons = [
  ...
  {
    id: "global-market-report",
    label: "Global Market Report",
    route: "/tools/trajectory/map",
    description: "Worldwide market analysis and regional opportunity mapping",
  },
];

interface TrajectoryButtonProps {
  button: (typeof trajectoryButtons)[0];
  index: number;
  mounted: boolean;
...
function TrajectoryButton({ button, index, mounted, hoveredButton, setHoveredButton, router }: TrajectoryButtonProps) {
    ...
    >
      ...
      >
        {circumference > 0 && radius > 0 && (
          ...
          >
            <motion.circle
              ...
              initial={false}
              animate={{ strokeDashoffset: hoveredButton === button.id ? 0 : circumference }}
        ...
        <div className="space-y-3 text-center relative z-10">
          ...
          <div className="w-8 h-0.5 bg-gradient-to-r from-blue-500 to-cyan-400 opacity-50 group-hover:opacity-100 transition-opacity duration-300 mx-auto" />
        </div>

        <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none z-0">
          <div className="absolute top-1/4 left-1/4 w-40 h-40 bg-blue-500/15 rounded-full blur-3xl -translate-y-1/2" />
          <div className="absolute bottom-1/4 right-1/4 w-40 h-40 bg-cyan-500/15 rounded-full blur-3xl translate-y-1/2" />
          <div className="absolute inset-0 bg-gradient-to-br from-blue-500/10 via-transparent to-cyan-500/10 rounded-full" />
        </div>
      </div>
    </motion.button>
  );
...
export default function TrajectoryHubPage() {
    ...
    <ProtectedRoute>
      <div className="relative min-h-screen bg-black text-white">
        <div className="fixed inset-0 -z-10 overflow-hidden pointer-events-none">
          <div className="absolute -top-32 -right-24 w-72 h-72 bg-blue-500/10 rounded-full blur-3xl" />
          <div className="absolute -bottom-28 -left-24 w-72 h-72 bg-cyan-500/10 rounded-full blur-3xl" />
        </div>
        ...
        <main className="relative z-10 py-16 sm:py-20">
          <section className="relative mx-auto max-w-4xl px-6 sm:px-8">
            <motion.div
            ...
            >
              <p className="text-gray-400 text-sm sm:text-base leading-relaxed">
                Explore the strategic trajectory roadmap or the global market opportunity report
              </p>
            </motion.div>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 sm:gap-6 max-w-2xl mx-auto">
              {trajectoryButtons.map((button, index) => (
                <TrajectoryButton
                  key={button.id}
                  button={button}
                  index={index}
