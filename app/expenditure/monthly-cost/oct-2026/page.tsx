...
const OCT_CATEGORIES: OctoberCategory[] = [
  {
    ...
    color: "#3B82F6",
    description:
      "The software stack remains unchanged, indicating that tools selected earlier are sufficient and fit-for-purpose. October focuses on maintaining operational efficiency.",
    includes: [
      "ChatGPT Business ×2",
      "HubSpot Starter ×2",
      "Canva Pro",
  ...
  },
  {
    ...
    color: "#8B5CF6",
    description:
      "Rental continues to be more capital-efficient than ownership at this stage. No upgrades or additional devices required in October.",
  ...
  },
  {
    ...
    color: "#EF4444",
    description:
      "Content cadence and production quality remain consistent. October maintains steady output aligned with boutique positioning.",
  ...
  },
  {
    ...
    color: "#F59E0B",
    description:
      "October is the most critical month of the year for HNI, private client, and corporate advisory decisions. Clients actively finalise trust structures, banking relationships, tax positioning, and cross-border arrangements ahead of year-end. Advertising spend is intentionally at its peak to capture high-intent demand, accelerate inbound deal flow, and support conversion-focused campaigns. This spend is execution-driven, not experimental, and directly aligned with closing probability.",
  },
  {
...
export default function October2026MonthlyCostPage() {
  const router = useRouter();
    const [hoveredIndex, setHoveredIndex] = useState<number | null>(null);
  const [lineHoverIndex, setLineHoverIndex] = useState<number | null>(null);

  const totalInr = OCT_CATEGORIES.reduce((sum, c) => sum + c.inr, 0);
  const totalUsd = OCT_CATEGORIES.reduce((sum, c) => sum + c.usd, 0);
  ...
  useEffect(() => {
    ...
    const initChart = () => {
      ...
      if ((window as any)._monthlyChart_OCT_CATEGORIES) {
        ((window as any)._monthlyChart_OCT_CATEGORIES).destroy();
      }
      
      const el = document.getElementById('pie-chart') as HTMLCanvasElement;
      if (!el) return false;
      
      (window as any)._monthlyChart_OCT_CATEGORIES = new (window as any).Chart(el.getContext('2d'), {
        type: 'doughnut',
        data: {
          labels: OCT_CATEGORIES.map(c => c.name),
          datasets: [{
            data: OCT_CATEGORIES.map(c => c.inr),
            backgroundColor: OCT_CATEGORIES.map(c => c.color),
            borderColor: '#0F172A',
            borderWidth: 2,
            hoverOffset: 4
          }]
    ...
    return () => {
      ...
      if ((window as any)._monthlyChart_OCT_CATEGORIES) {
        ((window as any)._monthlyChart_OCT_CATEGORIES).destroy();
        (window as any)._monthlyChart_OCT_CATEGORIES = null;
      }
    }
  }, []);
  
  const pieSlices = OCT_CATEGORIES.map(cat => ({
        ...
        <header className="relative z-10 border-b border-white/10 bg-black/90">
          <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-4 px-4 py-4 sm:px-6 lg:px-8">
            ...
            <div className="text-center flex-1">
              ...
              >
                October 2026 — Monthly Expenditure Breakdown
              </h1>
            </div>
        ...
        <main className="relative z-10 mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8 space-y-10">
            ...
            <div className="grid gap-6 md:grid-cols-[1.6fr,1.1fr]">
              <div className="space-y-4 text-sm sm:text-base leading-relaxed text-white/80">
                <p className="text-white/80">
                  October 2026 represents the peak year-end planning and decision-making window.
                  October is the most critical month of the year for HNI, private client, and
                  corporate advisory decisions. Clients actively finalise trust structures,
                  banking relationships, tax positioning, and cross-border arrangements ahead of
                  year-end.
                </p>
                <p className="text-white/80">
                  Advertising spend is intentionally at its peak to capture high-intent demand,
                  accelerate inbound deal flow, and support conversion-focused campaigns. This
                  spend is execution-driven, not experimental, and directly aligned with closing
                  probability. All other categories remain stable, indicating operational maturity
                  and fit-for-purpose infrastructure.
                </p>
              </div>
              <div className="grid gap-3 text-sm">
                <div className="rounded-2xl border border-white/10 bg-black/40 p-4">
                  <p className="text-xs uppercase tracking-[0.3em] text-white/60">
                    Total Month Cost (October 2026)
                  </p>
                  <p
                ...
                <div className="rounded-2xl border border-white/10 bg-black/40 p-4">
                  ...
                  <p className="mt-2 text-sm text-white/80">
                    No new licensing or legal costs. Marketing spend at peak to capture
                    high-intent demand and accelerate inbound deal flow. Travel & networking remain variable.
                  </p>
                </div>
              </div>
            </div>
          ...
          <section className="grid gap-8 lg:grid-cols-[1.2fr,1.1fr]">
            ...
            >
              ...
              <p className="text-xs text-white/60 mb-6">
                Animated month-level allocation of October burn across operating
                categories.
              </p>
                  ...
                  {hoveredIndex !== null && (
                    <div className="mt-3 rounded-2xl border border-white/15 bg-black/80 p-3 text-[11px] sm:text-xs text-white/80">
                      <p className="mb-1 font-semibold" style={{ fontFamily: "var(--font-benzin)" }}>
                        {OCT_CATEGORIES[hoveredIndex].name}
                      </p>
                      <p>{OCT_CATEGORIES[hoveredIndex].description}</p>
                    </div>
                  )}
                </div>
              </div>
          ...
          >
            <h2
              ...
              style={{ fontFamily: "var(--font-benzin)" }}
            >
              October Cost Curve by Category
            </h2>
            <p className="text-xs text-white/60 mb-6">
              Professional line visualization of how October spend builds up across
            ...
            <div className="w-full max-w-3xl mx-auto">
              <div className="relative h-64 w-full">
                ...
                {lineHoverIndex !== null && (
                  ...
                  >
                    <p
                      ...
                      style={{ fontFamily: "var(--font-benzin)" }}
                    >
                      {OCT_CATEGORIES[lineHoverIndex].name}
                    </p>
                    <p className="mb-1">
                      {formatInr(OCT_CATEGORIES[lineHoverIndex].inr)} ·{" "}
                      {formatUsd(OCT_CATEGORIES[lineHoverIndex].usd)}
                    </p>
                    <p className="text-white/70">
                      {OCT_CATEGORIES[lineHoverIndex].description}
                    </p>
                  </div>
                )}
                ...
                <svg viewBox="0 0 320 180" className="h-full w-full">
                  ...
                  {(() => {
                    const max = totalInr;
                    const step = (310 - 32) / (OCT_CATEGORIES.length - 1 || 1);
                    const points = OCT_CATEGORIES.map((cat, idx) => {
                      const x = 32 + step * idx;
                    ...
                    return (
                      <>
                        ...
                        {points.map((p, idx) => (
                          <motion.circle
                            key={OCT_CATEGORIES[idx].name}
                            cx={p.x}
                            cy={p.y}
                            r={4}
                            fill={OCT_CATEGORIES[idx].color}
                            stroke="#0F172A"
                            strokeWidth={1}
              ...
              <div className="mt-4 flex flex-wrap justify-between gap-4 text-[10px] sm:text-xs text-white/60">
                <p>
                  October shows peak year-end planning: marketing spend at peak to capture
                  high-intent demand and accelerate inbound deal flow, while all other categories
                  remain stable.
                </p>
                <p>
                  Travel & networking expenses remain variable and are incurred only
                  when value-accretive, reported octarately from fixed costs.
                </p>
              </div>
            </div>
          </motion.section>
          ...
          >
            <h2
              ...
              style={{ fontFamily: "var(--font-benzin)" }}
            >
              Category-by-Category View — October 2026
            </h2>
                  ...
                  {category.name === "Marketing & Ads" && (
                    <div className="rounded-xl border border-emerald-400/30 bg-emerald-400/10 p-3">
                      <p className="text-[10px] text-emerald-200 mb-2">
                        <span className="font-semibold">Change vs September:</span>{" "}
                        ₹10,000 increase
                      </p>
                      <p className="text-[10px] text-emerald-200/90 leading-relaxed">
                        October is the most critical month of the year for HNI, private client,
                        and corporate advisory decisions. Advertising spend is intentionally at
                        its peak to capture high-intent demand, accelerate inbound deal flow, and
            ...
            <div className="mt-6 rounded-2xl border border-blue-400/30 bg-blue-400/10 p-4">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-white/70">
                    Variable | Not fixed. October may include selective client meetings
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-white/60 mb-2 italic">
                    Frequently observed during peak advisory months with outsourced execution
                  </p>
                  <p className="text-xs text-white/70 mb-2">
                    Any such expenses would be deal-triggered, approved internally, and reported
                    transparently to investors:
                  </p>
                  <ul className="space-y-1 text-[11px] text-white/70 ml-4 list-disc">
                    <li>Accelerated third-party onboarding fees (banks, trustees, registrars)</li>
                    <li>Specialist legal or structuring opinions for complex year-end mandates</li>
                    <li>Increased travel costs for final-stage client and counterparty meetings</li>
                    <li>Urgent documentation, compliance, or due-diligence charges</li>
                  </ul>
                </div>
              </div>
            </div>
          ...
          <section className="grid gap-8 lg:grid-cols-[1.2fr,1.1fr]">
            ...
            >
              <h2
                ...
                style={{ fontFamily: "var(--font-benzin)" }}
              >
                October 2026 — Category-Wise Spend Table
              </h2>
                ...
                <table className="w-full text-xs sm:text-sm">
                  ...
                  <tbody>
                    ...
                    <tr className="border-t border-white/10 text-white font-semibold">
                      <td className="px-4 py-2">Total (October 2026)</td>
                      <td className="px-4 py-2 text-right">
                        {formatInr(totalInr)}
                      </td>
                      <td className="px-4 py-2 text-right">
            ...
            >
              <p className="text-xs uppercase tracking-[0.4em] text-white/60 mb-2">
                Investor Summary — October 2026
              </p>
              <p className="text-sm text-white/80 leading-relaxed">
                October reflects a transition month where decision-makers begin re-engaging
